...
class ClautodLogicLayerUser(Singleton):
    ...
    def user_get(self, given_user):
        # TODO
        pass

    def user_login(self, given_user):
        """
        ...
        :return: A signed JWT to be used as that user's session token
        """

        # Get the real user from the DB
        try:
            self.log.verbose("Finding username <%s> in database", given_user.username)
            user_from_db = self.database_layer.user_facility.user_get_by_username(given_user.username)
        ...
        user_from_login = User(given_user.username, given_user.password, user_from_db.password_salt)
        ...
        session_token = "I AM A USER TOKEN FOR <%s>" % user_from_db.username # TODO
        self.log.verbose("Authenticated user <%s>. Issuing token.", given_user.username, session_token)
