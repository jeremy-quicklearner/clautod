...
# Check parameters
if [ ! $1 ] ; then
    echo "[clautod-release] Specify version string in X.X.X format"
    exit 1
fi
if [[ ! $1 =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] ; then
    echo "[clautod-release] Version string <"$1"> is invalid. Use X.X.X format"
    exit 1
fi
...
echo "[clautod-release] Releasing clautod v"$1"..."
echo "[clautod-release] Preparing package tree..."
